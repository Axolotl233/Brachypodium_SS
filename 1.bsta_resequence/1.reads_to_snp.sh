...
#genotype and merge
java -jar /data/00/user/user112/software/gatk3.8-1/GenomeAnalysisTK.jar -T GenotypeGVCFs -R genome.fasta -V sample.gvcf.gz -V ...(ohter_sample) -o ./Pop.vcf.gz
java -jar /data/00/user/user112/software/gatk3.8-1/GenomeAnalysisTK.jar -T SelectVariants -R genome.fasta -V ./Pop.vcf.gz -selectType SNP -o ./Pop.SNP.vcf.gz
java -jar /data/00/user/user112/software/gatk3.8-1/GenomeAnalysisTK.jar -T SelectVariants -R genome.fasta -V ./Pop.vcf.gz -selectType INDEL -o ./Pop.INDEL.vcf.gz
java -jar /data/00/user/user112/software/gatk3.8-1/GenomeAnalysisTK.jar -T VariantFiltration -R genome.fasta -V ./Pop.SNP.vcf.gz  --filterExpression "QD < 2.0 || FS > 60.0 || MQ < 40.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0"  --filterName "my_snp_filter" -o ./Pop.HDflt.SNP.vcf.gz
java -jar /data/00/user/user112/software/gatk3.8-1/GenomeAnalysisTK.jar -T VariantFiltration -R genome.fasta -V ./Pop.INDEL.vcf.gz --filterExpression "QD < 2.0 || FS > 200.0 || ReadPosRankSum < -20.0" --filterName "my_indel_filter" -o ./Pop.HDflt.INDEL.vcf.gz
perl /data/00/user/user112/code/script/GATK.remove.hdfilter.pl --input ./Pop.HDflt.SNP.vcf.gz --out ./Pop.HDflted.SNP.vcf.gz --type SNP --marker my_snp_filter --multi
perl /data/00/user/user112/code/script/GATK.remove.hdfilter.pl --input ./Pop.HDflt.INDEL.vcf.gz --out ./Pop.HDflted.INDEL.vcf.gz --type INDEL --marker my_indel_filter
